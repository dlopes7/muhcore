...
from battlenet import EquippedItem
from muh_core_app.models import Equipamento, Bis, Boss
from wowheadhelper import get_boss_drops
...
def create_bosses():
    ...
    for boss_id, boss_nome in ids_bosses.items():
        try:
            Boss.objects.get(identificador=boss_id)
...
def criar_equipamento(equipamento_id):
...
def criar_bis(classe, spec, lista_bis):
...
django.setup()
connection = battlenet.Connection(public_key='nm3jrgp8avwjpqnptby38z763t9afyes',
...
bis_list_t17 = {'Shaman - Elemental': [113904, 113960, 115579, 113872, 115576, 120078, 115577, 113968,
                ...
                'Warlock - Demonology': [113869, 115586, 113872, 115589, 113937, 115588, 119332, 113876,
                                         113941, 115587, 113864, 113975, 118306, 113984, 113948]}
...
if build_bis:
    for classe, specs in classes.items():
        for spec in specs:
            print(classe, spec)
            bis_spec_list = bis_list_t17[classe + ' - ' + spec]

            for equipamento_id in bis_spec_list:
                equip_exists = criar_equipamento(equipamento_id)

            criar_bis(classe, spec, bis_spec_list)
            print()
